const origin = globalThis.ORIGIN_URL ?? "http://43-135-51-214.sslip.io/focus-room";
const blockedUserAgentPattern = /(MicroMessenger|miniprogram|MiniProgram|XWEB|com\.tencent|QQ\/|QQBrowser|MQQBrowser|XiaoHongShu|XHS|BytedanceWebview|Aweme|Toutiao|BiliApp|Weibo)/i;
...
const worker = {
  async fetch(request) {
    ...
    if (targetUrl.pathname.startsWith("/focus-room/_next/static/")) {
      headers.set("cache-control", "public, max-age=31536000, immutable");
    } else if (targetUrl.pathname.startsWith("/focus-room/api/")) {
...
function createTargetUrl(url) {
  ...
  if (requestUrl.pathname === "/room" || requestUrl.pathname === "/app") {
    targetUrl.pathname = originPath;
    targetUrl.search = requestUrl.search;
    return targetUrl;
  }
...
<head>
  ...
  <title>vibe studying room</title>
  <style>
...
<body>
  <main class="panel">
    ...
    <input value="${escapedUrl}" readonly onclick="this.select()" style="width:100%;margin:0 0 14px;padding:12px 14px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:#fff;font-size:14px" />
